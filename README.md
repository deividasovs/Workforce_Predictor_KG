...
```bash
deployed-model-sam$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
deployed-model-sam$ sam local start-api
deployed-model-sam$ curl http://localhost:3000/classify_digit
```
...
```bash
deployed-model-sam$ sam logs -n InferenceFunction --stack-name deployed-model-sam --tail
```
...
```bash
aws cloudformation delete-stack --stack-name deployed-model-sam
```
